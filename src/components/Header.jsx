    ...
    <header
      className={`fixed left-0 right-0 top-0 z-50 transition-all duration-300 ${
        isScrolled || isMobileMenuOpen
          ? "bg-photo-black/95 py-3 backdrop-blur-xl"
          : "py-5 md:py-6"
      }`}
    >
        ...
        <div className="hidden items-center gap-8 md:flex">
          {navLinks.map((link, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`transition-colors duration-300 ${active ? "text-white" : "text-zinc-400 group-hover:text-white"}`}
                >
                  {link.name}
                </span>
                <span
            ...
            <div className="mx-auto flex max-w-6xl flex-col px-6 py-2">
              {navLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`h-4 w-0.5 rounded-full bg-gradient-to-b from-photo-red to-photo-cyan transition-opacity duration-300 ${active ? "opacity-100" : "opacity-0"}`}
                    />
                    <span
                      className={`transition-colors ${active ? "text-white" : "text-zinc-300 group-hover:text-photo-cyan"}`}
                    >
                      {link.name}
                    </span>
                  </a>
